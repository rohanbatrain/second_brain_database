    ...
    def record_operation(
        ...
        if operation_type in ['hit', 'miss'] and response_time_ms > 100:
            logger.warning(
                ...
                operation_type, response_time_ms, operation.token_hash_prefix
            )
        
        if not success:
            logger.error(
                "Failed cache operation: %s for token %s",
                operation_type, operation.token_hash_prefix
            )
    ...
    async def perform_health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if recent_metrics.avg_response_time_ms > 100:
                health_status["alerts"].append(f"Average response time is high: {recent_metrics.avg_response_time_ms}ms")
