    ...
    async def get_database_health(self) -> DatabaseHealth:
        ...
        try:
            ...
            recommendations = self._generate_health_recommendations(
                total_tokens, active_tokens, revoked_tokens, stale_tokens,
                index_health, collection_sizes
            )
    ...
    async def run_full_maintenance(self) -> List[MaintenanceStats]:
        ...
        try:
            ...
            logger.info(
                ...
                total_duration, total_cleaned, total_errors
            )
    ...
    def _generate_health_recommendations(
        ...
        if revoked_tokens > 0 and revoked_tokens / max(total_tokens, 1) > 0.3:
            recommendations.append(
                f"High number of revoked tokens ({revoked_tokens}). Consider running cleanup."
            )
        ...
        if collection_sizes.get(AUDIT_COLLECTION, 0) > 100000:
            recommendations.append(
                "Large audit log collection. Consider implementing log rotation or archival."
            )
        
        if collection_sizes.get(USAGE_ANALYTICS_COLLECTION, 0) > 50000:
            recommendations.append(
                "Large usage analytics collection. Consider data aggregation or cleanup."
            )
