...
class PermanentTokenAnalytics:
    ...
    async def update_token_usage(
        ...
        try:
            ...
            await analytics_collection.insert_one(usage_record)
            
            logger.debug("Updated usage statistics for token: %s", token_hash[:8])
            return True
    ...
    async def get_token_usage_stats(self, token_id: str) -> Optional[TokenUsageStats]:
        ...
        try:
            ...
            # Count total uses
            total_uses = await audit_collection.count_documents({
                "token_id": token_id,
            ...
            is_active = (
                last_used_at is not None and 
                days_since_last_use is not None and 
                days_since_last_use <= INACTIVE_TOKEN_THRESHOLD_DAYS
            )
            
            is_stale = (
                last_used_at is None or 
            ...
            risk_score = self._calculate_risk_score(
                unique_ips, unique_user_agents, total_uses, days_since_creation
            )
            ...
            logger.error("Error getting token usage stats: %s", e)
            return None
    ...
    async def get_user_usage_analytics(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            tokens_collection = db_manager.get_collection("permanent_tokens")
            user_tokens = await tokens_collection.find({
                "user_id": user_id,
                "is_revoked": False
            }).to_list(length=None)
            ...
            logger.error("Error getting user usage analytics: %s", e)
    ...
    async def get_system_usage_analytics(self) -> UsageAnalytics:
        ...
        try:
            # Check cache first
            if (self.cached_analytics and self.last_analytics_update and 
                datetime.utcnow() - self.last_analytics_update < timedelta(minutes=self.cache_ttl_minutes)):
            ...
            audit_collection = db_manager.get_collection(AUDIT_COLLECTION)
            unique_ips_cursor = audit_collection.distinct("ip_address", {
                "event_type": "token_validated",
                "ip_address": {"$ne": None}
            })
