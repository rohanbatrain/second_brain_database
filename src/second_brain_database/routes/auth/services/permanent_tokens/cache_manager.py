...
async def warm_user_token_cache(user_id: str) -> int:
    ...
    try:
        ...
        recent_tokens = await tokens_collection.find({
            "user_id": user_id,
            "is_revoked": False,
            "last_used_at": {"$gte": cutoff_date}
        }).limit(CACHE_WARM_BATCH_SIZE).to_list(length=None)
        ...
        logger.info("Cache warmed for user %s: %d tokens cached", user_id, cached_count)
        return cached_count
        
    except Exception as e:
        logger.error("Error warming cache for user %s: %s", user_id, e)
        return 0
...
async def warm_frequently_used_tokens() -> int:
    ...
    try:
        ...
        pipeline = [
            ...
            {
                "$match": {
                    "token_count": {"$gte": CACHE_WARM_MIN_USAGE}
                }
            },
            {
