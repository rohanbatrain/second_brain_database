...
from second_brain_database.routes.auth.services.abuse.notification import (
    notify_user_of_suspicious_reset)
...
async def detect_password_reset_abuse(email: str, ip: str) -> AbuseDetectionResult:
    """
    Detects abuse for password reset requests.
    ...
    Returns:
        AbuseDetectionResult: Dict with 'suspicious', 'reasons', and 'ip_reputation'.

    Side Effects:
        Logs events to MongoDB, stores flags in Redis, and may notify users.
    """
    ...
    try:
        ...
        if len(recent_requests) >= MAX_RESET_REQUESTS:
            ...
            logger.info("Self-abuse detected for %s from %s", email, ip)
        if len(unique_ips) >= MAX_RESET_UNIQUE_IPS:
            ...
            logger.info("Targeted abuse detected for %s from %s", email, ip)
        # IP reputation check
        try:
            import httpx
            ...
            logger.debug("IPinfo checked for %s: %s", ip, ip_reputation)
        except ImportError:
            logger.error("IPinfo check failed for %s (ImportError)", ip, exc_info=True)
            ip_reputation = None
        except (AttributeError, TypeError, ValueError, KeyError, json.JSONDecodeError):
            logger.error("IPinfo check failed for %s (data error)", ip, exc_info=True)
            ip_reputation = None
        except httpx.RequestError:
            logger.error("IPinfo check failed for %s (request error)", ip, exc_info=True)
            ip_reputation = None
        except httpx.HTTPStatusError:
            logger.error("IPinfo check failed for %s (HTTP status error)", ip, exc_info=True)
            ip_reputation = None
        ...
        if suspicious:
            ...
            try:
                ...
                logger.info("Flagged and notified for %s, %s", email, ip)
