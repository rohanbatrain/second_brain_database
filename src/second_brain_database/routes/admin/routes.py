...
from second_brain_database.routes.admin.models import (
    AbuseEvent, EmailIpPair, AbuseEventResolveRequest
    )
...
async def admin_list_abuse_events_api(
    ...
    logger.debug("Admin requested abuse events: email=%s, event_type=%s, resolved=%s, limit=%d", 
                 email, event_type, resolved, limit)
    events = await admin_list_abuse_events(
        email=email, event_type=event_type, resolved=resolved, limit=limit
        )
