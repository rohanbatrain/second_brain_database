...
class UpdateWalletPermissionsRequest(BaseModel):
    member_permissions: Dict[str, Dict[str, Any]] = Field(
        ..., description="Dictionary mapping user_id to their permissions"
    )
...
async def create_workspace(req: CreateWorkspaceRequest, current_user: dict = Depends(get_current_user_dep)):
    """
    Create a new workspace.

    This endpoint creates a new workspace with the authenticated user as the owner and an administrator.
    A team wallet is automatically initialized for the workspace, allowing for immediate use of shared funds.

    Args:
        req (CreateWorkspaceRequest): The request body containing the name and optional description of the workspace.
        current_user (dict): The authenticated user, injected by Depends.

    Returns:
        WorkspaceResponse: The newly created workspace object, including its ID, owner, members, and wallet status.
    """
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Failed to auto-initialize wallet for workspace {workspace_data['workspace_id']}: {str(wallet_error)}"
            )
...
async def get_my_workspaces(current_user: dict = Depends(get_current_user_dep)):
    ...
    This endpoint retrieves a list of all workspaces that the authenticated user belongs to.

    Args:
        current_user (dict): The authenticated user, injected by Depends.

    Returns:
        List[WorkspaceResponse]: A list of workspace objects.
    """
    user_id = str(current_user["_id"])
...
async def get_workspace(workspace_id: str, current_user: dict = Depends(get_current_user_dep)):
    """
    Get details for a single workspace.

    This endpoint retrieves the details of a specific workspace, provided the authenticated user is a member.
    ...
    Returns:
        WorkspaceResponse: The workspace object.
    """
    user_id = str(current_user["_id"])
    try:
...
async def update_workspace(
    workspace_id: str, req: UpdateWorkspaceRequest, current_user: dict = Depends(get_current_user_dep)
):
    """
    Update a workspace's details.

    This endpoint allows an administrator of a workspace to update its name, description, and settings.

    Args:
        workspace_id (str): The ID of the workspace to update.
        req (UpdateWorkspaceRequest): The request body containing the fields to update.
        current_user (dict): The authenticated user, injected by Depends.

    Returns:
        WorkspaceResponse: The updated workspace object.
    """
...
async def delete_workspace(workspace_id: str, current_user: dict = Depends(get_current_user_dep)):
    """
    Delete a workspace.

    This endpoint allows the owner of a workspace to permanently delete it.
    ...
    admin_user_id = str(current_user["_id"])
    try:
        deleted = await workspace_manager.delete_workspace(workspace_id=workspace_id, admin_user_id=admin_user_id)
        if not deleted:
            raise HTTPException(
...
async def add_workspace_member(
    workspace_id: str, req: AddMemberRequest, current_user: dict = Depends(get_current_user_dep)
):
    """
    Add a new member to a workspace.

    This endpoint allows an administrator of a workspace to add a new member and assign them a role.

    Args:
        workspace_id (str): The ID of the workspace to add the member to.
        req (AddMemberRequest): The request body containing the user ID of the member to add and their role.
        current_user (dict): The authenticated user, injected by Depends.

    Returns:
        WorkspaceResponse: The updated workspace object, including the new member.
    """
    admin_user_id = str(current_user["_id"])
    try:
        workspace = await workspace_manager.add_member(
            workspace_id=workspace_id, admin_user_id=admin_user_id, user_id_to_add=req.user_id_to_add, role=req.role
        )
        workspace["created_at"] = workspace["created_at"].isoformat()
        workspace["updated_at"] = workspace["updated_at"].isoformat()
        # Check if wallet is initialized
...
async def remove_workspace_member(workspace_id: str, user_id: str, current_user: dict = Depends(get_current_user_dep)):
    """
    Remove a member from a workspace.

    This endpoint allows an administrator of a workspace to remove a member.
    The owner of the workspace cannot be removed.

    Args:
        workspace_id (str): The ID of the workspace to remove the member from.
        user_id (str): The ID of the user to remove.
        current_user (dict): The authenticated user, injected by Depends.

    Returns:
        WorkspaceResponse: The updated workspace object, without the removed member.
    """
    admin_user_id = str(current_user["_id"])
    try:
        workspace = await workspace_manager.remove_member(
            workspace_id=workspace_id, admin_user_id=admin_user_id, user_id_to_remove=user_id
        )
        workspace["created_at"] = workspace["created_at"].isoformat()
        workspace["updated_at"] = workspace["updated_at"].isoformat()
        # Check if wallet is initialized
...
async def update_workspace_member_role(
    ...
    This endpoint allows an administrator of a workspace to change the role of a member.
    The available roles are `admin`, `editor`, and `viewer`.

    Args:
        workspace_id (str): The ID of the workspace.
        user_id_to_update (str): The ID of the user whose role is to be updated.
        req (UpdateMemberRoleRequest): The request body containing the new role.
        current_user (dict): The authenticated user, injected by Depends.
...
async def initialize_team_wallet(
    request: Request, workspace_id: str, current_user: dict = Depends(get_current_user_dep)
):
    """
    ...
    await security_manager.check_rate_limit(
        request, f"team_wallet_init_{user_id}", rate_limit_requests=5, rate_limit_period=3600
    )
    ...
    try:
        ...
        try:
            ...
            if workspace:
                ...
                raise HTTPException(
                    ...
                    detail={"error": "INSUFFICIENT_PERMISSIONS", "message": "You are not a member of this workspace"},
                )
            else:
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail={"error": "INSUFFICIENT_PERMISSIONS", "message": "Only workspace admins can initialize the wallet"},
        )
    except Exception as e:
        if hasattr(e, "error_code") and e.error_code == "WALLET_ALREADY_EXISTS":
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail={"error": e.error_code, "message": str(e)}
            )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"error": "INTERNAL_SERVER_ERROR", "message": str(e)},
...
async def get_team_wallet(request: Request, workspace_id: str, current_user: dict = Depends(get_current_user_dep)):
    """
    ...
    # Apply rate limiting
    await security_manager.check_rate_limit(
        request, f"team_wallet_get_{user_id}", rate_limit_requests=30, rate_limit_period=3600
    )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail={"error": "INSUFFICIENT_PERMISSIONS", "message": "You are not a workspace member"},
        )
        ...
        if error_code == "WALLET_NOT_INITIALIZED":
            ...
            try:
                ...
                raise HTTPException(
                    ...
                    detail={
                        "error": error_code,
                        "message": "Team wallet not initialized and auto-initialization failed",
                    },
                )
        ...
        logger = logging.getLogger(__name__)
        logger.error(
            f"Unexpected error in get_team_wallet for workspace {workspace_id}, user {user_id}: {str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.post(
    "/{workspace_id}/wallet/token-requests", response_model=TokenRequestResponse, status_code=status.HTTP_201_CREATED
)
async def create_token_request(
    ...
    """
    Create a request for SBD tokens from the team account.

    This endpoint allows a workspace member to request SBD tokens from the shared team wallet.
    If the requested amount is below the user's auto-approval limit, the request is
    processed immediately. Otherwise, it is submitted for an administrator's approval.

    Args:
        request (Request): The incoming request object.
        workspace_id (str): The ID of the workspace.
        token_request (CreateTokenRequestRequest): The request body containing the amount and reason for the request.
        current_user (dict): The authenticated user, injected by Depends.

    Returns:
        TokenRequestResponse: The created token request object, including its status.
    """
    user_id = str(current_user["_id"])

    # Apply rate limiting
    await security_manager.check_rate_limit(
        request, f"team_token_request_{user_id}", rate_limit_requests=10, rate_limit_period=3600
    )

    try:
        request_data = await team_wallet_manager.create_token_request(
            workspace_id=workspace_id, user_id=user_id, amount=token_request.amount, reason=token_request.reason
        )
        # Convert datetime objects to ISO strings
        ...
        if error_code in ["WALLET_NOT_INITIALIZED", "ACCOUNT_FROZEN", "VALIDATION_ERROR"]:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail={"error": error_code, "message": str(e)}
            )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"error": "INTERNAL_SERVER_ERROR", "message": str(e)},
...
async def get_pending_token_requests(
    ...
    This endpoint retrieves a list of all SBD token requests that are pending approval
    from an administrator. This is an admin-only endpoint.
    ...
    Returns:
        List[TokenRequestResponse]: A list of pending token request objects.
    """
    user_id = str(current_user["_id"])

    # Apply rate limiting
    await security_manager.check_rate_limit(
        request, f"team_pending_requests_{user_id}", rate_limit_requests=30, rate_limit_period=3600
    )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail={"error": "INSUFFICIENT_PERMISSIONS", "message": "Only workspace admins can view pending requests"},
        )
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def review_token_request(
    ...
    """
    Review a pending SBD token request.

    This endpoint allows a workspace administrator to approve or deny a pending token request from a member.

    Args:
        request (Request): The incoming request object.
        workspace_id (str): The ID of the workspace.
        request_id (str): The ID of the token request to review.
        review_request (ReviewTokenRequestRequest): The request body containing the action (`approve` or `deny`) and optional comments.
        current_user (dict): The authenticated user, injected by Depends.

    Returns:
        dict: A dictionary confirming the action taken and the updated status of the request.
    """
    user_id = str(current_user["_id"])

    # Apply rate limiting
    await security_manager.check_rate_limit(
        request, f"team_token_review_{user_id}", rate_limit_requests=20, rate_limit_period=3600
    )

    try:
        result = await team_wallet_manager.review_token_request(
            request_id=request_id, admin_id=user_id, action=review_request.action, comments=review_request.comments
        )
        ...
        logger = logging.getLogger(__name__)
        logger.error(
            f"Error reviewing token request {request_id} for workspace {workspace_id} by user {user_id}: {str(e)}",
            exc_info=True,
        )
        ...
        if error_code in ["TOKEN_REQUEST_NOT_FOUND", "VALIDATION_ERROR"]:
            ...
            logger.warning(
                f"Token request review failed with 403 error: user {user_id} not authorized for workspace {workspace_id}"
            )
...
async def update_spending_permissions(
    ...
    This endpoint allows a workspace administrator to set or update the spending permissions
    for one or more members of the workspace.

    Args:
        request (Request): The incoming request object.
        workspace_id (str): The ID of the workspace.
        permissions (UpdateWalletPermissionsRequest): The request body containing a dictionary
            mapping user IDs to their new permissions.
        current_user (dict): The authenticated user, injected by Depends.

    Returns:
        WalletPermissionsResponse: A dictionary containing the updated permissions for each member.
    """
    user_id = str(current_user["_id"])

    # Apply rate limiting
    await security_manager.check_rate_limit(
        request, f"team_wallet_permissions_{user_id}", rate_limit_requests=5, rate_limit_period=3600
    )
    ...
    try:
        ...
        for member_user_id, member_perms in permissions.member_permissions.items():
            result = await team_wallet_manager.update_spending_permissions(
                workspace_id=workspace_id, admin_id=user_id, user_id=member_user_id, permissions=member_perms
            )
        ...
        raise HTTPException(
            ...
            detail={
                ...
                "message": "Only workspace admins can modify spending permissions",
            },
        )
    except Exception as e:
        error_code = getattr(e, "error_code", "INTERNAL_SERVER_ERROR")
...
async def freeze_unfreeze_account(
    ...
    """
    Freeze or unfreeze the team wallet account.

    This endpoint allows a workspace administrator to freeze or unfreeze the team's shared SBD token account.
    When an account is frozen, no spending is allowed, but deposits can still be made.

    Args:
        request (Request): The incoming request object.
        workspace_id (str): The ID of the workspace.
        freeze_request (FreezeAccountRequest): The request body containing the action (`freeze` or `unfreeze`) and an optional reason.
        current_user (dict): The authenticated user, injected by Depends.

    Returns:
        dict: A dictionary confirming the action taken and the updated status of the account.
    """
    admin_id = str(current_user["_id"])

    # Apply rate limiting
    await security_manager.check_rate_limit(
        request, f"team_wallet_freeze_{admin_id}", rate_limit_requests=3, rate_limit_period=3600
    )
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail={
                "error": "INSUFFICIENT_PERMISSIONS",
                "message": "Only workspace admins can freeze/unfreeze accounts",
            },
        )
    except Exception as e:
        error_code = getattr(e, "error_code", "INTERNAL_SERVER_ERROR")
...
async def get_team_audit_trail(
    ...
    # Apply rate limiting
    await security_manager.check_rate_limit(
        request, f"team_audit_access_{user_id}", rate_limit_requests=10, rate_limit_period=3600
    )

    try:
        audit_trail = await team_wallet_manager.get_team_audit_trail(
            workspace_id=workspace_id, admin_id=user_id, start_date=start_date, end_date=end_date, limit=limit
        )
        return audit_trail
    except WorkspaceNotFound:
        raise HTTPException(
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail={"error": "INSUFFICIENT_PERMISSIONS", "message": "Only workspace admins can access audit trails"},
        )
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def generate_compliance_report(
    ...
    # Apply rate limiting
    await security_manager.check_rate_limit(
        request, f"team_compliance_report_{user_id}", rate_limit_requests=5, rate_limit_period=3600
    )
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail={
                "error": "INSUFFICIENT_PERMISSIONS",
                "message": "Only workspace admins can generate compliance reports",
            },
        )
    except Exception as e:
        error_code = getattr(e, "error_code", "INTERNAL_SERVER_ERROR")
...
async def designate_backup_admin(
    ...
    if not backup_admin_id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={"error": "VALIDATION_ERROR", "message": "backup_admin_id is required"},
        )

    # Apply rate limiting
    await security_manager.check_rate_limit(
        request, f"team_backup_admin_{user_id}", rate_limit_requests=3, rate_limit_period=3600
    )

    try:
        result = await team_wallet_manager.designate_backup_admin(
            workspace_id=workspace_id, admin_id=user_id, backup_admin_id=backup_admin_id
        )
        return result
    except WorkspaceNotFound:
        raise HTTPException(
        ...
        raise HTTPException(
            ...
            detail={
                "error": "INSUFFICIENT_PERMISSIONS",
                "message": "Only workspace admins can designate backup admins",
            },
        )
    except Exception as e:
        error_code = getattr(e, "error_code", "INTERNAL_SERVER_ERROR")
...
async def remove_backup_admin(
    request: Request, workspace_id: str, backup_admin_id: str, current_user: dict = Depends(get_current_user_dep)
):
    """
    ...
    # Apply rate limiting
    await security_manager.check_rate_limit(
        request, f"team_remove_backup_admin_{user_id}", rate_limit_requests=3, rate_limit_period=3600
    )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail={"error": "INSUFFICIENT_PERMISSIONS", "message": "Only workspace admins can remove backup admins"},
        )
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def emergency_unfreeze_account(
    ...
    if not emergency_reason or len(emergency_reason.strip()) < 10:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={"error": "VALIDATION_ERROR", "message": "Emergency reason must be at least 10 characters"},
        )

    # Apply rate limiting (stricter for emergency operations)
    await security_manager.check_rate_limit(
        request, f"team_emergency_unfreeze_{user_id}", rate_limit_requests=1, rate_limit_period=3600
    )

    try:
        result = await team_wallet_manager.emergency_unfreeze_account(
            workspace_id=workspace_id, backup_admin_id=user_id, emergency_reason=emergency_reason.strip()
        )
        return result
    except WorkspaceNotFound:
        raise HTTPException(
        ...
        raise HTTPException(
            ...
            detail={
                "error": "INSUFFICIENT_PERMISSIONS",
                "message": "Only designated backup admins can perform emergency unfreezes",
            },
        )
    except Exception as e:
        error_code = getattr(e, "error_code", "INTERNAL_SERVER_ERROR")
