...
async def update_profile(request: Request, data: dict = Body(...), current_user: dict = Depends(enforce_all_lockdowns)):
    ...
    logger.info("[%s] POST /profile/update - User: %s, IP: %s, User-Agent: %s", request_id, username, client_ip, user_agent[:100])
    ...
    try:
        ...
        log_error_with_context(e, context={"user": username, "ip": client_ip, "request_id": request_id}, operation="update_profile")
...
async def get_profile_info(request: Request, current_user: dict = Depends(enforce_all_lockdowns)):
    ...
    logger.info("[%s] GET /profile/info - User: %s, IP: %s, User-Agent: %s", request_id, username, client_ip, user_agent[:100])
    ...
    try:
        ...
        profile_fields = [
            "user_first_name", "user_last_name", "user_username", "user_email", "user_dob", "user_gender", "user_bio"
        ]
        ...
        log_error_with_context(e, context={"user": username, "ip": client_ip, "request_id": request_id}, operation="get_profile_info")
        return JSONResponse({"status": "error", "detail": "Internal server error", "error": str(e)}, status_code=500)
