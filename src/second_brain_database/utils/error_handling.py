    ...
    for attempt in range(config.max_attempts):
        try:
            ...
            if config.retryable_exceptions and not any(
                ...
                logger.info(
                    "Non-retryable exception (not in retryable list) for %s: %s",
                    context.operation, str(e)
                )
                raise
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            try:
                ...
                duration = time.time() - start_time
                logger.info(
                    "Operation %s (sync) completed successfully in %.3fs",
                    operation_name, duration
                )
                return result
    ...
    try:
        ...
        try:
            ...
            logger.error(
                ...
                context.operation, str(fallback_error)
            )
