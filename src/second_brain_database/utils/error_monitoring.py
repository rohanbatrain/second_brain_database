...
class ErrorMonitor:
    ...
    async def record_error(
        ...
        self.logger.info(
            ...
            event.error_type, event.operation, event.severity.value,
        ...
        return {
            ...
            "recovery_statistics": {
                "total_recovery_attempts": sum(
                    1 for e in self.error_events if e.recovery_attempted
                ),
                "successful_recoveries": sum(
                    1 for e in self.error_events if e.recovery_successful
                ),
...
async def record_error_event(
    ...
    await error_monitor.record_error(
        error, context, severity, recovery_attempted, recovery_successful
    )
