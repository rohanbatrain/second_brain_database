...
class RateLimit(BaseModel):
    ...
    def validate_hourly_limit(cls, v, values):
        ...
        if 'requests_per_minute' in values:
            min_hourly = values['requests_per_minute'] * 10  # At least 10 minutes worth
            if v < min_hourly:
...
class SecurityConfig(BaseModel):
    ...
    allowed_permissions: List[str] = Field(
        ...
        description="List of allowed permissions for MCP access"
    )
    ...
    audit_enabled: bool = Field(
        default=True,
        description="Whether to enable audit logging for MCP operations"
    )
    ip_whitelist: Optional[List[str]] = Field(
        default=None,
        description="Optional IP whitelist for MCP access"
    )
...
class MCPServerConfig(BaseModel):
    ...
    description: str = Field(
        default="Second Brain Database MCP Server",
        max_length=500,
...
class MCPToolConfig(BaseModel):
    ...
    permissions: List[str] = Field(
        default_factory=list,
        description="Required permissions for tool access"
    )
    rate_limit_action: Optional[str] = Field(
        default=None,
        description="Rate limiting action key"
    )
    audit_enabled: bool = Field(
        default=True,
        description="Whether to audit this tool's usage"
    )
...
class MCPResourceConfig(BaseModel):
    ...
    permissions: List[str] = Field(
        default_factory=list,
        description="Required permissions for resource access"
    )
...
class MCPPromptConfig(BaseModel):
    ...
    permissions: List[str] = Field(
        default_factory=list,
        description="Required permissions for prompt access"
    )
