...
async def designate_backup_admin(
    ...
    """
    Designate a family member as backup admin for emergency access and continuity.
    
    Args:
        family_id: The ID of the family
        member_id: The ID of the member to designate as backup admin
        emergency_contact_info: Optional emergency contact information
        
    Returns:
        Dictionary containing backup admin designation confirmation
        
    Raises:
        MCPAuthorizationError: If user is not a family admin or owner
        MCPValidationError: If member is not found or already a backup admin
    """
    user_context = get_mcp_user_context()
    
    # Validate admin access to family (preferably owner for backup admin designation)
    await require_family_access(family_id, required_role="admin", user_context=user_context)
    
    # Prevent self-designation
    if member_id == user_context.user_id:
        raise MCPValidationError("Cannot designate yourself as backup admin")
    ...
    try:
        ...
        # Designate backup admin using existing manager
        result = await family_manager.designate_backup_admin(
            family_id=family_id,
            admin_id=user_context.user_id,
            backup_admin_id=member_id,
        ...
        await create_mcp_audit_trail(
            ...
            changes={
                "backup_admin_id": member_id,
                "backup_admin_username": member_info.get("username"),
                "emergency_contact_provided": bool(emergency_contact_info)
            },
            metadata={
                "designated_by": user_context.user_id,
                "previous_role": member_info.get("role")
            }
        )
        
        logger.info("Designated backup admin %s for family %s by user %s", 
                   member_id, family_id, user_context.user_id)
        return result
        
    except Exception as e:
        logger.error("Failed to designate backup admin %s for family %s: %s", member_id, family_id, e)
...
async def remove_backup_admin(
    ...
    """
    Remove backup admin designation from a family member.
    ...
    Raises:
        MCPAuthorizationError: If user is not a family admin or owner
        MCPValidationError: If backup admin is not found
    """
    user_context = get_mcp_user_context()
    ...
    try:
        # Get backup admin info before removal
        members = await family_manager.get_family_members(family_id, user_context.user_id)
        backup_admin_info = next((m for m in members if m.get("user_id") == backup_admin_id), None)
        
        if not backup_admin_info:
            raise MCPValidationError("Backup admin not found in family")
        
        # Remove backup admin designation using existing manager
        ...
        await create_mcp_audit_trail(
            ...
            changes={
                ...
                "removed_backup_admin_username": backup_admin_info.get("username"),
                "reason": reason
            },
            metadata={
                "removed_by": user_context.user_id,
                "new_role": "member"  # Assuming they revert to member role
            }
        )
        
        logger.info("Removed backup admin %s from family %s by user %s", 
                   backup_admin_id, family_id, user_context.user_id)
        return result
        
    except Exception as e:
        logger.error("Failed to remove backup admin %s from family %s: %s", backup_admin_id, family_id, e)
