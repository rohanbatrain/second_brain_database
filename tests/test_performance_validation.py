...
class PerformanceValidator:
    ...
    async def cleanup(self):
        ...
        try:
            ...
            await db_manager.get_collection("permanent_token_usage_analytics").delete_many({"user_id": self.test_user_id})
    ...
    async def test_database_performance(self):
        ...
        start_time = time.time()
        active_tokens = await collection.find({
            "user_id": self.test_user_id,
            "is_revoked": False
        }).to_list(length=None)
        ...
        self.performance_results['database'] = {
            'queries': dict(query_times),
